...
(function($, window, document, undefined) {
    ...
    $.fn.imageuploadify = function(opts) {
        ...
        this.each(function() {
            ...
            let counter = 0;
            let dragbox = $(`<div class="imageuploadify well"><div class="imageuploadify-overlay"><i class="fa fa-picture-o"></i></div><div class="imageuploadify-images-list text-center"><i class="ri-upload-cloud-2-line"></i><label for='signatureAlbumFiles' class='imageuploadify-message'>Drag&Drop Your File(s)Here </br>or</label><button type="button" class="btn btn-default">Select file(s)</button></div></div>`);
            ...
            const retrieveFiles = (files) => {
                for (let index = 0; index < files.length; ++index) {
                    if (!accept || compareMimeType(result, files[index].type, /(?:\.([^.]+))?$/.exec(files[index].name)[1])) {
                        ...
                        readingFile(id, files[index], index, files, elementid);
                        totalFiles.push({
                            id: id,
                            file: files[index]
                        });
            ...
            const readingFile = (id, file, dataIndex, filesList, elementid) => {
                let dataFileArray= filesList;
                const fReader = new FileReader();
                const width = dragbox.width();
                const boxesNb = Math.floor(width / 100);
                const marginSize = Math.floor((width - (boxesNb * 100)) / (boxesNb + 1));
                let container = $(`<div class='imageuploadify-container'><button type='button'class='btn btn-danger ri-close-circle-line' id="${dataIndex}"></button><div class='imageuploadify-details'><span>${file.name}</span><span>${file.type}</span><span>${file.size}</span></div></div>`);
            ...
            const refreshFiles = () => {
                ...
                for (let i = 0; i < totalFiles.length; i++) {
                    fileBuffer.items.add(totalFiles[i].file);
                }
                setTimeout(function(){
                     $('#'+elementid).prop('files', fileBuffer.files);
                }, 300)
